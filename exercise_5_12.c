...
void entab(char buf[], int len, int spaces, int columns);
void detab(char buf[], int len, int spaces, int columns);

void print_help() {
    printf("usage:\n");
    printf("    ./a.out <entab/detab> [number of spaces] [-m <number of columns>]\n");
}
...
int main(int argc, char *argv[]) {
    ...
    while ((len = my_getline(line, MAXLINE)) > 0) {
        ...
        case OPERATION_ENTAB:
            entab(line, len, spaces_in_tab, columns);
            break;
        case OPERATION_DETAB:
            detab(line, len, spaces_in_tab, columns);
            break;
        default:
